...
async def iikoProblem(calliiProb: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "iiko"')
    tg_analytic.statistics(calliiProb.message.chat.id, 'iiko')
...
async def ErrorsToOrderStatusesAndProgramOperation(callETOSAPO: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Ошибки связанные со статусами заказов и работы программы"')
    tg_analytic.statistics(callETOSAPO.message.chat.id, 'Ошибки связанные со статусами заказов и работы программы')
...
async def ErrorsTheOpeningAndClosingOfTheCashRegisterShift(callETOACOTCRS: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Ошибки связанные с открытием и закрытием кассовой смены"')
    tg_analytic.statistics(callETOACOTCRS.message.chat.id, 'Ошибки связанные с открытием и закрытием кассовой смены')
...
async def WhenCloseProgramDoesNotClose(callWCPDNC: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "При закрытии программы, она начинает висеть и не закрывается."')
    tg_analytic.statistics(callWCPDNC.message.chat.id, 'При закрытии программы, она начинает висеть и не закрывается.')
...
async def AYellowMessageAppeared(callAYMA: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Появилось сообщение желтого цвета, в верхнем левом углу экрана в IIKO"')
    tg_analytic.statistics(callAYMA.message.chat.id, 'Появилось сообщение желтого цвета, в верхнем левом углу экрана в IIKO')
...
async def TheRequiredProductIsNotInTheList(callTRPINITL: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Нет необходимого товара в списке/поиске при формировании актов списания и пр."')
    tg_analytic.statistics(callTRPINITL.message.chat.id, 'Нет необходимого товара в списке/поиске при формировании актов списания и пр.')
...
async def TheChefInIikoSousChefDoesNotSeeOrders(callTCIISCDNSO: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Повар в iikoSousChef не видит заказов. Хотя ранее все работало."')
    tg_analytic.statistics(callTCIISCDNSO.message.chat.id,'Повар в iikoSousChef не видит заказов. Хотя ранее все работало.')
...
async def FailedToSetCashierName(callFTSCN: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "При открытии кассовой смены выдает ошибку "Не удалось задать имя кассира..."')
    tg_analytic.statistics(callFTSCN.message.chat.id,'При открытии кассовой смены выдает ошибку "Не удалось задать имя кассира...')
...
async def HoursHaveExpiredYouNeedToCloseShift(callHHEYNTCS: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "При попытке оплатить заказ выдаёт ошибку "Истекли 24 часа, необходимо закрыть смену"')
    tg_analytic.statistics(callHHEYNTCS.message.chat.id,'При попытке оплатить заказ выдаёт ошибку "Истекли 24 часа, необходимо закрыть смену')
...
async def EmployeeCantWorkHere(callECWH: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "При попытке Авторизации пишет: «Нет ИНН», «Нет сотрудника», «Сотрудник не может тут работать»"')
    tg_analytic.statistics(callECWH.message.chat.id,'При попытке Авторизации пишет: «Нет ИНН», «Нет сотрудника», «Сотрудник не может тут работать»')
...
async def screen(callscreen: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Экран покупателя"')
    tg_analytic.statistics(callscreen.message.chat.id,'Экран покупателя')
...
async def NoPicturesBlackScreen(callNPBS: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Нет картинок. Черный экран"')
    tg_analytic.statistics(callNPBS.message.chat.id, 'Нет картинок. Черный экран')
...
async def NoPicturesWhiteScreen(callNPWS: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Нет картинок. Белый экран"')
    tg_analytic.statistics(callNPWS.message.chat.id, 'Нет картинок. Белый экран')
...
async def ErrorOnTheScreenBuyer(callEOTSB: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "На экране покупателя ошибка «Нет сигнала»"')
    tg_analytic.statistics(callEOTSB.message.chat.id, 'На экране покупателя ошибка «Нет сигнала»')
...
async def ArchivingProgram(callAProgram: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Архивная программа"')
    tg_analytic.statistics(callAProgram.message.chat.id, 'Архивная программа')
...
async def ElectronicQueueAndTV(callElQATV: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Электронная очередь и ТВ"')
    tg_analytic.statistics(callElQATV.message.chat.id, 'Электронная очередь и ТВ')
...
async def ServerUnavailableMessage(callSULM: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Касса недоступна"')
    tg_analytic.statistics(callSULM.message.chat.id, 'Касса недоступна')
...
async def SettingUpAnElectronicQueue(callSUAEQ):
    ...
    logger.debug('Пользователь нажал кнопку "Настройка электронной очереди при открытии новой точки или при замене телевизора"')
    tg_analytic.statistics(callSUAEQ.message.chat.id, 'Настройка электронной очереди при открытии новой точки или при замене телевизора')
...
async def InternetConnectionNotWorkingOnTV(callICNWOT: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "На телевизоре не работает интернет соединение"')
    tg_analytic.statistics(callICNWOT.message.chat.id, 'На телевизоре не работает интернет соединение')
...
async def CCTV(callCCTV: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Видеонаблюдение"')
    tg_analytic.statistics(callCCTV.message.chat.id, 'Видеонаблюдение')
...
async def FRButton(callsFRButton: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Касса и Фискальный регистратор"')
    tg_analytic.statistics(callsFRButton.message.chat.id, 'Касса и Фискальный регистратор')
...
async def ProblemsPay(callsPP: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Проблемы при попытке оплатить заказ"')
    tg_analytic.statistics(callsPP.message.chat.id, 'Проблемы при попытке оплатить заказ')
...
async def CashBoxProblems(callsCBP: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Проблемы с кассой (после оплаты и при открытии смены)"')
    tg_analytic.statistics(callsCBP.message.chat.id, 'Проблемы с кассой (после оплаты и при открытии смены)')
...
async def OtherFR(callOFR: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Другое"')
    tg_analytic.statistics(callOFR.message.chat.id, 'Другое')
...
async def JammedCarvingKnife(callJCK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Заклинил нож авторезчика"')
    tg_analytic.statistics(callJCK.message.chat.id, 'Заклинил нож авторезчика')
...
async def JammedCarvingKnifeSecond(callJCKS: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Заклинил нож авторезчика 2"')
    tg_analytic.statistics(callJCKS.message.chat.id, 'Заклинил нож авторезчика 2')
...
async def DisableAutoCutter(callDAC: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Отключить авторезчик"')
    tg_analytic.statistics(callDAC.message.chat.id, 'Отключить авторезчик')
...
async def NoConnectionWDevice(callsNCWD: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Нет связи с устройством"')
    tg_analytic.statistics(callsNCWD.message.chat.id, 'Нет связи с устройством')
...
async def FNStorage(callaFNStorage: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Исчерпан ресурс хранения ФН"')
    tg_analytic.statistics(callaFNStorage.message.chat.id, 'Исчерпан ресурс хранения ФН')
...
async def FailedSerialNumber(callsFSN: types.CallbackQuery):
    await callsFSN.message.edit_text(text='Выключить фискальный регистратор на кнопку и включить заново.\n'
                                             ...
                                             'Если вышеперечисленные действия не помогли - обратиться в техподдержку.', reply_markup=callTechSuppKeyboard)
    logger.debug('Пользователь нажал кнопку "Не удалось получить серийный номер устройства"')
    tg_analytic.statistics(callsFSN.message.chat.id, 'Не удалось получить серийный номер устройства')
...
async def CloseShift(callsCShift: types.CallbackQuery):
    await callsCShift.message.edit_text(text='Вчера сотрудники забыли закрыть кассовую смену.\n'
                                           'Закрываем кассовую смену через iiko.\n'
                                            'Текущий заказ можно удалить и пробить в актуальной смене.', reply_markup=callTechSuppKeyboard)
    logger.debug('Пользователь нажал кнопку "Истекли 24 часа, необходимо закрыть смену"')
    tg_analytic.statistics(callsCShift.message.chat.id, 'Истекли 24 часа, необходимо закрыть смену')
...
async def FailedOperatingMode(callFOM: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Не удалось установить режим работы (Произошла ошибка -3822)"')
    tg_analytic.statistics(callFOM.message.chat.id, 'Не удалось установить режим работы (Произошла ошибка -3822)')
...
async def FailedToCloseCheck(callFTC: types.CallbackQuery):
    ...
    logger.debug('Не удалось закрыть чек (Произошла ошибка -4015)"')
    tg_analytic.statistics(callFTC.message.chat.id, 'Не удалось закрыть чек (Произошла ошибка -4015)')
...
async def FailedToGetPaymentMethod(callFTGM: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Не удалось получить способ оплаты"')
    tg_analytic.statistics(callFTGM.message.chat.id, 'Не удалось получить способ оплаты')
...
async def NotSupportedDeviceMode(callNSDM: types.CallbackQuery):
    await callNSDM.message.edit_text(text='Необходимо выключить и включить фискальный регистратор и перезагрузить программу IIKO.\n'
                                           'Если вышеперечисленные действия не помогли - обратиться в техподдержку.', reply_markup=callTechSuppKeyboard)
    logger.debug('Пользователь нажал кнопку "Не поддерживается в данном режиме устройства"')
    tg_analytic.statistics(callNSDM.message.chat.id, 'Не поддерживается в данном режиме устройства')
...
async def FailedToSetName(callFTSN: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "При открытии кассовой смены выдает ошибку ,,Не удалось задать имя кассира...,,"')
    tg_analytic.statistics(callFTSN.message.chat.id, 'При открытии кассовой смены выдает ошибку ,,Не удалось задать имя кассира...,,')
...
async def NoConnectionToOFD(callNCTOFD: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "После оплаты на кассе всплывает уведомление ,, Нет связи с ОФД. Колличество неотправленных докуметов: 1"')
    tg_analytic.statistics(callNCTOFD.message.chat.id, 'После оплаты на кассе всплывает уведомление ,, Нет связи с ОФД. Колличество неотправленных докуметов: 1')
...
async def ErrorCheque(callEC: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "После оплаты на кассе всплывает уведомление ,, При печати фискального чека произошла ошибка (-3807) ,,"')
    tg_analytic.statistics(callEC.message.chat.id, 'После оплаты на кассе всплывает уведомление ,, При печати фискального чека произошла ошибка (-3807) ,,')
...
async def Internet(callInternet: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Интернет"')
    tg_analytic.statistics(callInternet.message.chat.id, 'Проблемы с интернетом')
...
async def WIFI(callWIFI: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Wi-Fi"')
    tg_analytic.statistics(callWIFI.message.chat.id, 'Wi-Fi')
...
async def OtherInternet(callOtherInternet: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Другое"')
    tg_analytic.statistics(callOtherInternet.message.chat.id, 'Другое интернет')
...
async def InternetOnCashbox(callIOC: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Интернет на кассе"')
    tg_analytic.statistics(callIOC.message.chat.id, 'Интернет на кассе')
...
async def YellowTriangle(callYT: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Желтый треугольник"')
    tg_analytic.statistics(callYT.message.chat.id, 'Желтый треугольник')
...
async def RedCross(callRCB: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Красный крестик, сетевой кабель не подключен"')
    tg_analytic.statistics(callRCB.message.chat.id, 'Красный крестик, сетевой кабель не подключен')
...
async def InternetConnectionStatus(callICS: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Статус интернет соединения"')
    tg_analytic.statistics(callICS.message.chat.id, 'Статус интернет соединения')
...
async def NoConnectionsAvailable(callNCA: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Нет доступных подключений"')
    tg_analytic.statistics(callNCA.message.chat.id, 'Нет доступных подключений')
...
async def WithoutInternetAccess(callWIA: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Без доступа к Интернету"')
    tg_analytic.statistics(callWIA.message.chat.id, 'Без доступа к Интернету')
...
async def CashlessPayment(callCP: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Безналичная оплата. Проблемы с оплатой заказа"')
    tg_analytic.statistics(callCP.message.chat.id, 'Безналичная оплата. Проблемы с оплатой заказа')
...
async def ProblemWithPaymentOrders(callPWPO: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Проблема с оплатой заказов"')
    tg_analytic.statistics(callPWPO.message.chat.id, 'Проблема с оплатой заказов')
...
async def ProblemsAfterOrderPayment(callPAOP: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Проблемы возникшие после оплаты заказа"')
    tg_analytic.statistics(callPAOP.message.chat.id, 'Проблемы возникшие после оплаты заказа')
...
async def ProblemsPaymentForTheOrder(callPPFTO: types.CallbackQuery):
    ...
    tg_analytic.statistics(callPPFTO.message.chat.id, 'Проблемы связанные с безналичной оплатой заказа')
...
async def ProblemsPaymentForTheOrder(callPPFTO: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Нет связи с устройством"')
    tg_analytic.statistics(callPPFTO.message.chat.id, 'Нет связи с устройством')
...
async def FailedToGetDeviceSerialNumber(callFTGDN: types.CallbackQuery):
    await callFTGDN.message.edit_text(text='Выключить фискальный регистратор на кнопку и включить заново.\n'
                                        ...
                                        'Если вышеперечисленные действия не помогли, то свяжитесь с техподдержкой', reply_markup=callTechSuppKeyboard)
    logger.debug('Пользователь нажал кнопку "Не удалось получить серийный номер устройства"')
    tg_analytic.statistics(callFTGDN.message.chat.id, 'Не удалось получить серийный номер устройства')
...
async def FNStorageResourceExhausted(callFNSRE: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Исчерпан ресурс хранения ФН"')
    tg_analytic.statistics(callFNSRE.message.chat.id, 'Исчерпан ресурс хранения ФН')
...
async def FailedToSetOperatingMode(callFTSOM: types.CallbackQuery):
    await callFTSOM.message.edit_text(text='Вчера сотрудники забыли закрыть кассовую смену.\n'
                                        ...
                                        'Текущий заказ можно удалить и пробить в актуальной смене.', reply_markup=callTechSuppKeyboard)
    logger.debug('Пользователь нажал кнопку "Не удалось установить режим работы (Произошла ошибка -3822)"')
    tg_analytic.statistics(callFTSOM.message.chat.id, 'Не удалось установить режим работы (Произошла ошибка -3822)')
...
async def FailedToCloseCheck(callFTCC: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Не удалось закрыть чек (Произошла ошибка -4015)"')
    tg_analytic.statistics(callFTCC.message.chat.id, 'Не удалось закрыть чек (Произошла ошибка -4015)')
...
async def FailedToGetPaymentMethod(callFTGPM: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Не удалось получить способ оплаты"')
    tg_analytic.statistics(callFTGPM.message.chat.id, 'Не удалось получить способ оплаты')
...
async def NotSupportedInThisDeviceMode(callNSITDM: types.CallbackQuery):
    await callNSITDM.message.edit_text(text='Необходимо выключить и включить фискальный регистратор и перезагрузить программу IIKO.\n'
                                         'Если вышеперечисленные действия не помогли, то свяжитесь с техподдержкой', reply_markup=callTechSuppKeyboard)
    logger.debug('Пользователь нажал кнопку "Не поддерживается в данном режиме устройства"')
    tg_analytic.statistics(callNSITDM.message.chat.id, 'Не поддерживается в данном режиме устройства')
...
async def WPayingForTheOrderPrepaymentWindowAppears(callWPFTOPWA: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "При оплате заказа появляется окно предоплаты"')
    tg_analytic.statistics(callWPFTOPWA.message.chat.id, 'При оплате заказа появляется окно предоплаты')
...
async def NoConnectionToOFDNumberOfUnsentDocuments(callNCTOFDNOUD: types.CallbackQuery):
    await callNCTOFDNOUD.message.edit_text(text='Такое уведомление ВСЕГДА появляется после каждого закрытого заказа. \n'
                                           'Если в данном уведомлении количество документов больше, чем ,,1,,,  необходимо обратиться в техническую поддержку.', reply_markup=callTechSuppKeyboard)
    logger.debug('Пользователь нажал кнопку "После оплаты на кассе всплывает уведомление "Нет связи с ОФД. Колличество неотправленных докуметов: 1"')
    tg_analytic.statistics(callNCTOFDNOUD.message.chat.id, 'После оплаты на кассе всплывает уведомление "Нет связи с ОФД. Колличество неотправленных докуметов: 1')
...
async def AnErrorOccurredWhilePrintingTheReceipt(callAEOWPTR: types.CallbackQuery):
    await callAEOWPTR.message.edit_text(text='Ошибка говорит о том, что фискальный регистратор не видет чековую ленту, либо она отстутсвует\n'
                                           ...
                                           'Далее закройте крышку и повторите оплату заказа.', reply_markup=callTechSuppKeyboard)
    logger.debug('Пользователь нажал кнопку "После оплаты на кассе всплывает уведомление "При печати фискального чека произошла ошибка (-3807)"')
    tg_analytic.statistics(callAEOWPTR.message.chat.id, 'После оплаты на кассе всплывает уведомление "При печати фискального чека произошла ошибка (-3807)')
...
async def PaymentTypeBankCardDoesNotWork(callPTBCDNW: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Не работает тип оплаты "Банковская карта"')
    tg_analytic.statistics(callPTBCDNW.message.chat.id, 'Не работает тип оплаты "Банковская карта')
...
async def BankTerminalNotWorkingBlackScreen(callBTNWBS: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Не работает банковский терминал, черный экран"')
    tg_analytic.statistics(callBTNWBS.message.chat.id, 'Не работает банковский терминал, черный экран')
...
async def ThereIsNoBankCardPayment(callTINBCP: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Нет кнопки оплаты "Баковская карта""')
    tg_analytic.statistics(callTINBCP.message.chat.id, 'Нет кнопки оплаты "Баковская карта"')
...
async def Delivery(callDelivery: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Доставка"')
    tg_analytic.statistics(callDelivery.message.chat.id, 'Доставка')
...
async def NoPermissionToSell(callNPTS: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "У продукта ... нет разрешения продаваться в выбраной ценовой категории"')
    tg_analytic.statistics(callNPTS.message.chat.id, 'У продукта ... нет разрешения продаваться в выбраной ценовой категории')
...
async def DisabledOnPoint(callDOP: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Тип оплаты ... отключен на точке"')
    tg_analytic.statistics(callDOP.message.chat.id, 'Тип оплаты ... отключен на точке')
...
async def CannotBeTransferred(callCBT: types.CallbackQuery):
    await callCBT.message.edit_text(text='Необходимо обратиться в службу доставки по тел. 8-800-700-67-76',
                                    reply_markup=callTechSuppKeyboard)
    logger.debug('Пользователь нажал кнопку "Продукт ... не может быть передан в IIIKO"')
    tg_analytic.statistics(callCBT.message.chat.id, 'Продукт ... не может быть передан в IIIKO')
...
async def PointOfSaleNotSynced(callPOSNS: types.CallbackQuery):
    await callPOSNS.message.edit_text(text='Необходимо обратиться в службу доставки по тел. 8-800-700-67-76',
                                      reply_markup=callTechSuppKeyboard)
    logger.debug('Пользователь нажал кнопку "Не синхронизирована торговая точка IIKO программой доставки"')
    tg_analytic.statistics(callPOSNS.message.chat.id, 'Не синхронизирована торговая точка IIKO программой доставки')
...
async def RequestErrorInIIKO(callREII: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Ошибка запроса в IIKO (не передаются заказы)"')
    tg_analytic.statistics(callREII.message.chat.id, 'Ошибка запроса в IIKO (не передаются заказы)')
...
async def TheRequiredPaymentType(callTRPT: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Нужного типа оплаты нет в IIKO"')
    tg_analytic.statistics(callTRPT.message.chat.id, 'Нужного типа оплаты нет в IIKO')
...
async def RemoteNotWorking(callRNW: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Не работает удаленка"')
    tg_analytic.statistics(callRNW.message.chat.id, 'Не работает удаленка')
...
async def MailProblem(callMP: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "MailProblem"')
    tg_analytic.statistics(callMP.message.chat.id, 'MailProblem')
...
async def LicenseRestriction(callLR: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "LicenseRestriction"')
    tg_analytic.statistics(callLR.message.chat.id, 'LicenseRestriction')
...
async def NoAccessToTheServer(callNATHS: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "NoAccessToTheServer"')
    tg_analytic.statistics(callNATHS.message.chat.id, 'NoAccessToTheServer')
...
async def NoAccessToTheServerChain(callNATTSC: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "NoAccessToTheServerChain"')
    tg_analytic.statistics(callNATTSC.message.chat.id, 'NoAccessToTheServerChain')
...
async def ServerIsNotAnIIKO_RMS(callSINAIIKO: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "ServerIsNotAnIIKO_RMS"')
    tg_analytic.statistics(callSINAIIKO.message.chat.id, 'ServerIsNotAnIIKO_RMS')
...
async def TheRedLightIsOn(callTRLIO: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "TheRedLightIsOn"')
    tg_analytic.statistics(callTRLIO.message.chat.id, 'TheRedLightIsOn')
...
async def CartridgeReplacement(callCR: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "CartridgeReplacement"')
    tg_analytic.statistics(callCR.message.chat.id, 'CartridgeReplacement')
...
async def PaperJam(callPJ: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "PaperJam"')
    tg_analytic.statistics(callPJ.message.chat.id, 'PaperJam')
...
async def Other(callOther: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Другое"')
    tg_analytic.statistics(callOther.message.chat.id, 'Другое')
...
async def backToMainMenu(callbTMM: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToMainMenu"')
    tg_analytic.statistics(callbTMM.message.chat.id, 'backToMainMenu')
...
async def backToMainMenuKeyboard(callbTMMk: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToMainMenuKeyboard"')
    tg_analytic.statistics(callbTMMk.message.chat.id, 'backToMainMenuKeyboard')
...
async def backToIIKOKeyboard(callbtIIKO: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToIIKOKeyboard"')
    tg_analytic.statistics(callbtIIKO.message.chat.id, 'backToIIKOKeyboard')
...
async def backToErrorsToOrderStatusesAndProgramOperationKeyboard(callbtETOSAPOK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "Не поддерживается в данном режиме устройства"')
    tg_analytic.statistics(callbtETOSAPOK.message.chat.id, 'backToErrorsToOrderStatusesAndProgramOperationKeyboard')
...
async def backToErrorsTheOpeningAndClosingOfTheCashRegisterShiftKeyboard(callbtETOACOTCRSK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToErrorsTheOpeningAndClosingOfTheCashRegisterShiftKeyboard"')
    tg_analytic.statistics(callbtETOACOTCRSK.message.chat.id, 'backToErrorsTheOpeningAndClosingOfTheCashRegisterShiftKeyboard')
...
async def backToFRButtonKeyboard(callbtFRBK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToFRButtonKeyboard"')
    tg_analytic.statistics(callbtFRBK.message.chat.id, 'backToFRButtonKeyboard')
...
async def backToProblemsPayKeyboard(callbtPPK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToProblemsPayKeyboard"')
    tg_analytic.statistics(callbtPPK.message.chat.id, 'backToProblemsPayKeyboard')
...
async def backToCashBoxProblemsKeyboard(callbtCBPK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToCashBoxProblemsKeyboard"')
    tg_analytic.statistics(callbtCBPK.message.chat.id, 'backToCashBoxProblemsKeyboard')
...
async def backToOtherFRKeyboard(callbtOFRK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToOtherFRKeyboard"')
    tg_analytic.statistics(callbtOFRK.message.chat.id, 'backToOtherFRKeyboard')
...
async def backToscreenKeyboard(callbTSKK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToscreenKeyboard"')
    tg_analytic.statistics(callbTSKK.message.chat.id, 'backToscreenKeyboard')
...
async def backToCashlessPaymentKeyboard(callbtCPK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToCashlessPaymentKeyboard"')
    tg_analytic.statistics(callbtCPK.message.chat.id, 'backToCashlessPaymentKeyboard')
...
async def backToProblemsAfterOrderPaymentKeyboard(callbtPAOPK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToProblemsAfterOrderPaymentKeyboard"')
    tg_analytic.statistics(callbtPAOPK.message.chat.id, 'backToProblemsAfterOrderPaymentKeyboard')
...
async def backToProblemWithPaymentOrdersKeyboard(callbtPWPOK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToProblemWithPaymentOrdersKeyboard"')
    tg_analytic.statistics(callbtPWPOK.message.chat.id, 'backToProblemWithPaymentOrdersKeyboard')
...
async def backToProblemsPaymentForTheOrderKeyboard(callbtPPFTOK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToProblemsPaymentForTheOrderKeyboard"')
    tg_analytic.statistics(callbtPPFTOK.message.chat.id, 'backToProblemsPaymentForTheOrderKeyboard')
...
async def backToInternetKeyboard(callbtIK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToInternetKeyboard"')
    tg_analytic.statistics(callbtIK.message.chat.id, 'callNSDM')
...
async def backToInternetOnCashboxKeyboard(callbtIOCK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToInternetOnCashboxKeyboard"')
    tg_analytic.statistics(callbtIOCK.message.chat.id, 'backToInternetOnCashboxKeyboard')
...
async def backToOtherInternetKeyboard(callbtOIK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToOtherInternetKeyboard"')
    tg_analytic.statistics(callbtOIK.message.chat.id, 'backToOtherInternetKeyboard')
...
async def backToDeliveryKeyboard(callbtTDK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToDeliveryKeyboard"')
    tg_analytic.statistics(callbtTDK.message.chat.id, 'backToDeliveryKeyboard')
...
async def backToElectronicQueueAndTVKeyboard(callbtEQATVK: types.CallbackQuery):
    ...
    logger.debug('Пользователь нажал кнопку "backToElectronicQueueAndTVKeyboard"')
    tg_analytic.statistics(callbtEQATVK.message.chat.id, 'backToElectronicQueueAndTVKeyboard')
